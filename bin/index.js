...
program
  ...
  .action(async (projectName, options) => {
    ...
    getLoading.succeed('🎖️ 获取模板成功！当前模版来源：' + user.username)
...
program
  ...
  .action(async (commitText, options) => {
    ...
    if (!commitText && options.push) {
      ...
      pushLoading.succeed('🎖️ 推送 Git 成功！')
    } else {
      // coolbo commit 'test'
      if (!commitText) {
...
program
  ...
  .action(async () => {
    ...
    pullLoading.succeed('🎖️ 拉取 Git 成功！')
  })
...
program
  ...
  .action(async (options) => {
    ...
    if (Object.keys(options).length === 0) {
      let user = await getConfigFromFile()
      console.log('🎖️ 当前 Git 用户为：' + user.username + `${user.username === 'coolbo-cn' ? ' -> default' : ' -> custom'}`)
      return
    }
